...
PSD 模式复用现有 **AI 图片生成** 窗口，不新增独立向导、移动端标签页、`TaskType.PSD` 或 `AssetType.PSD`。当前工作台流程为：用户从素材库选择、上传、拖拽或粘贴参考图并输入提示词后，先创建 `TaskType.CHAT` 图层分析任务；用户在 PSD 工作台审阅、重命名、调整图层提示词或排除图层；确认后再为参与的栅格图层创建现有 `TaskType.IMAGE` 图层素材任务。导出结果是 `.psd-ready-workspace.zip` 工作区包，不宣称 OpenAI 兼容图片 API 或上游图片模型直接返回原生 `.psd`。
...
- PSD 源图入口必须支持素材库/媒体库选择、本地上传、拖拽和粘贴；无论来源如何，工作台只保留一张源图用于 `TaskType.CHAT` 分析，不创建 PSD 专用素材类型。
...
2. 从素材库选择原始海报或参考图，并分别抽查本地上传、拖拽、粘贴入口仍可替换源图；输入设计描述，点击分析/生成入口。
