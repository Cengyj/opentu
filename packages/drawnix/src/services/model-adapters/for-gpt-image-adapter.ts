...
import {
  ...
  LEGACY_FOR_GPT_IMAGE_GENERATION_REQUEST_SCHEMA,
...
type ForGPTResponseFormat = 'url' | 'b64_json';
...
export function buildForGPTImageRequestBody(
  request: ImageGenerationRequest
): Record<string, unknown> {
  const options = buildForGPTImageRequestOptions(request);
...
async function readErrorMessage(response: Response): Promise<string> {
  ...
  return `For GPT Image request failed: ${response.status}`;
}

function resolveForGPTImagePath(
  context: { binding?: { submitPath?: string } | null }
): string {
  return context.binding?.submitPath || '/images/generations';
}

export const forGPTImageAdapter: ImageModelAdapter = {
  ...
  async generateImage(context, request) {
    const response = await sendAdapterRequest(context, {
      path: resolveForGPTImagePath(context),
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(buildForGPTImageRequestBody(request)),
    });
...
export const registerForGPTImageAdapter = (): void => {
