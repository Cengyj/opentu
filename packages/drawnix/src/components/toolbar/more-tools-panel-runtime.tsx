...
import { ArrowLineShape, BasicShapes, type DrawPointerType } from '@plait/draw';
...
import type { ToolbarButtonConfig } from '../../types/toolbar-config.types';
...
// Plait's runtime accepts null as the sentinel that clears creation mode, but
// the published declaration currently omits that value from its parameter.
const setNullableCreationMode = setCreationMode as (
  board: PlaitBoard,
  mode: BoardCreationMode | null
) => void;
...
export interface MoreToolsPanelRuntimeProps {
  onMouseEnter: () => void;
  onMouseLeave: () => void;
}
...
const MoreToolsPanel: React.FC<MoreToolsPanelProps> = ({
  ...
  const handleMouseEnter = useCallback((popupKey: string) => {
    ...
    const currentTimeout = hoverTimeoutRef.current[popupKey];
    if (currentTimeout) {
  ...
  const onPointerDown = useCallback((pointer: DrawnixPointerType) => {
    ...
    if (pointer === BasicShapes.text) {
      setNullableCreationMode(board, null);
    } else {
...
export const MoreToolsPanelRuntime: React.FC<
  MoreToolsPanelRuntimeProps
