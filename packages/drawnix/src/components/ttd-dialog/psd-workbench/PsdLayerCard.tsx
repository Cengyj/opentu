...
export function PsdLayerCard({
  ...
  const canRetryLayer =
    ...
    layer.visible !== false &&
    (layerTaskState?.status === 'failed' ||
      layerTaskState?.status === 'cancelled');

  return (
    <article
      className={`psd-layer-card psd-layer-card--${layerStatus} ${
        isSelected ? 'psd-layer-card--selected' : ''
      } ${isHidden ? 'psd-layer-card--hidden' : ''} ${
        isExpanded ? 'psd-layer-card--expanded' : ''
      }`}
    >
      <div
        className="psd-layer-card__header"
        onClick={() => {
          onSelect();
          onToggleExpanded();
        }}
      >
        <div className="psd-layer-card__main">
          <span
            className={`psd-layer-card__swatch psd-layer-card__swatch--${layer.type}`}
          />
          <div className="psd-layer-card__info">
            <strong>{layer.name}</strong>
            <small>
              {getLayerTypeLabel(layer.type, uiLanguage)} · {layer.opacity}% ·{' '}
              {isHidden
                ? uiLanguage === 'zh'
                  ? '已排除'
                  : 'Excluded'
        ...
        <div
          className="psd-layer-card__right-actions"
          onClick={(event) => event.stopPropagation()}
        >
          <span
            className={`psd-layer-card__status psd-layer-card__status--${layerStatus}`}
          >
      ...
      {isExpanded ? (
        <div
          className="psd-layer-card__body"
          onClick={(event) => event.stopPropagation()}
        >
          <div className="psd-layer-card__fields">
            <label>
              ...
              <input
                ...
                value={layer.name}
                onChange={(event) =>
                  onLayerNameChange?.(layer.id, event.target.value)
                }
            ...
            <label>
              <span>
                {uiLanguage === 'zh' ? '图层生成提示词' : 'Layer prompt'}
              </span>
              <textarea
                aria-label={
                  uiLanguage === 'zh'
                ...
                onChange={(event) =>
                  onLayerPromptChange?.(layer.id, event.target.value)
                }
            ...
            <label className="psd-layer-card__include">
              <input
                ...
                aria-label={
                  uiLanguage === 'zh'
                    ? `参与生成与导出：${layer.name}`
                    : `Include in generation and export: ${layer.name}`
                }
              />
              <span>
                {uiLanguage === 'zh'
                  ? '参与生成与导出'
                  : 'Include in generation/export'}
              </span>
            </label>
          </div>

          <div className="psd-layer-card__metrics">
            <div className="psd-layer-card__metric-row">
              <span>{uiLanguage === 'zh' ? '类型' : 'Type'}</span>
              <strong>{getLayerTypeLabel(layer.type, uiLanguage)}</strong>
            </div>
            <div className="psd-layer-card__metric-row">
              <span>{uiLanguage === 'zh' ? '透明度' : 'Opacity'}</span>
              <strong>{layer.opacity}%</strong>
            </div>
            {canvasSize ? (
              <div className="psd-layer-card__metric-row">
                <span>{uiLanguage === 'zh' ? '画布大小' : 'Canvas size'}</span>
                <strong>
            ...
            {selectedLayerBounds ? (
              <div className="psd-layer-card__metric-row">
                <span>{uiLanguage === 'zh' ? '图层坐标' : 'Coordinates'}</span>
                <strong>
                  {Math.round(selectedLayerBounds.left)}%,{' '}
                  {Math.round(selectedLayerBounds.top)}%
                </strong>
              </div>
            ) : null}
          </div>

          {layerTaskState?.error ? (
            <div className="psd-layer-card__error">{layerTaskState.error}</div>
          ) : null}
          <div className="psd-layer-card__tip">
            <CheckCircle2 size={12} />
            <span>
              {uiLanguage === 'zh'
                ? 'PSD 语义：同画布、原坐标、透明背景，导入 Photoshop 无需移动缩放即可叠放还原。'
                : 'PSD semantics: same canvas, original coordinates, transparent background, and in-place Photoshop stacking.'}
            </span>
          </div>
        </div>
      ) : null}
