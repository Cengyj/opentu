...
- [x] 6.3 运行 `drawnix:typecheck`、`web:typecheck`、相关回归、cycles、`git diff --check`、`NX_DAEMON=false pnpm exec nx build web`、`NX_DAEMON=false pnpm exec nx build drawnix`、startup contract 与 `verify:startup`：类型检查/build/diff 均 exit 0，Drawnix JS 与 `vite-plugin-dts` 声明生成无 TypeScript diagnostics，0 cycles；最终 `drawnix-app` 481,924B、入口静态图 1,941,175B、所有单文件不超过 512,000B。全仓 lint 仍 exit 1，但当前 433 errors / 2,441 warnings 相对 HEAD 439 / 2,471 减少 6 / 30，本 change 新增 diagnostics 为 0
