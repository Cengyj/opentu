...
#### Scenario: 首屏保留 AI 输入布局并在空闲时自动升级

- **WHEN** 画布与 AI 输入轻壳尚未完成首次绘制
- **THEN** 页面可以显示与完整输入栏等尺寸的轻量输入壳
- **AND** 轻壳不初始化模型健康、工作流提交或生成任务副作用
- **AND** `isStartupOperable` 成立后，系统在浏览器空闲回调自动加载并挂载完整输入栏，不要求用户先点击轻壳
- **AND** 用户在空闲回调前聚焦、输入或通过快捷键激活时，系统取消待执行回调并通过同一个 loader 立即加载
- **AND** 自动挂载不得获取输入焦点或提交生成请求
