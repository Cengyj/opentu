...
- [ ] 1.7 Obtain user approval for provider-form/switch semantics, provider-scoped model-management semantics, and F-09 Chinese/English behavior.
...
- [ ] 3.1 Add failing tests for keyboard-operable model-group disclosure with programmatic expanded/controlled state and a separate benchmark action.
- [ ] 3.2 Add failing tests for one programmatically current discovery type filter and current selected model checkboxes.
- [ ] 3.3 Add failing tests for vendor expanded state and localized model test/remove action names without hover.
- [ ] 3.4 Implement native/equivalent semantics while preserving search, ordering, single-vendor expansion, selection, recommendation, clear, confirm, remove, and benchmark callbacks.
- [ ] 3.5 Prove no discovery, price, health, benchmark, storage, task, media, or canvas side effect is added by semantic navigation alone.
...
- [ ] 5.1 Run focused settings-dialog/model-discovery tests with exact files/tests/exits and zero external requests in interface fixtures.
- [ ] 5.2 Run focused ESLint and Drawnix typecheck, then repository gates against the recorded baseline.
- [ ] 5.3 Run available browser/E2E keyboard, assistive-technology, zh/en, normal/empty/loading/failure/discovery flows and classify missing browser binaries separately.
- [ ] 5.4 Check desktop/tablet/mobile, light/dark, 100%/200%, high-DPI, touch, reduced motion, long values, and slow/offline presentation where the environment supports them.
- [ ] 5.5 Capture matched before/after screenshots only after implementation; do not claim performance or visual improvement without required measurements.
- [ ] 5.6 Recheck all discovery/routing/storage/health/credential/benchmark/settings-nav/window changes for owner overlap before implementation.
- [x] 5.7 Attempt strict OpenSpec validation and, if the CLI remains unavailable, record the exit code plus manual file/scenario/name/owner checks without claiming strict validation. (`openspec` unavailable, exit 127; manual checks passed.)
