...
The reachable provider settings page visually labels its connection fields and provider-enabled switches, but the rendered controls do not expose those relationships. The initial production Chromium audit also included the experimental asynchronous-image switch and manual provider-pricing controls; `remove-provider-async-image-preference` and `remove-provider-pricing-controls` remove those controls, so they are no longer part of this change's implementation scope. The remaining switches express their on/off state only through CSS classes rather than `aria-checked`.
...
- Associate the remaining provider connection, image-compatibility, and credential fields with persistent localized labels and instructions.
- Give the existing provider-enabled switches localized names and programmatic checked state without changing their values or persistence callbacks.
...
- Affected code: `settings-dialog.tsx`, `model-discovery-dialog.tsx`, F-09-scoped i18n keys, focused component tests, and only necessary content styles
- Adjacent owners:
...
- The initial production `dist/apps/web` audit at 1280×720/DPR 1 rendered six provider/pricing inputs and two native selects. `remove-provider-pricing-controls` subsequently removes two inputs and their pricing-group selector; this change owns only the remaining controls.
- The initial run rendered three TDesign buttons with `role="switch"`, including the subsequently removed asynchronous-image preference. The remaining provider-enabled switches had no `aria-label`, `aria-labelledby`, or `aria-checked`; their current state was present only in `t-is-checked` CSS classes. Switching provider selection without changing or saving data reproduced the same result for both configured profiles.
...
- `settings-dialog.tsx` renders visible sibling labels without control relationships, while the provider rows render the remaining unnamed/class-state switches.
...
- `I18nProvider` wraps Drawnix and the application menu can change its `zh`/`en` state, but the remaining F-09 content modules do not consume `useI18n`; their application-authored normal, empty, loading, failure, form, action, and discovery copy follows fixed Chinese branches.
