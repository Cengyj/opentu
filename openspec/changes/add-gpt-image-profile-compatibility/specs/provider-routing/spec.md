...
- **AND** the first profile has image API compatibility `for-gpt-image`
...
#### Scenario: ForOpenCode auto inference selects dedicated For GPT mode

- **GIVEN** a provider profile uses a `foropencode.com` base URL
- **AND** the profile image API compatibility is `auto`
- **WHEN** the system infers image compatibility
- **THEN** it SHALL resolve to the For GPT image format
- **AND** SHALL keep the request eligible for the dedicated For GPT Image adapter path

#### Scenario: Generic OpenAI-compatible auto inference

- **GIVEN** a provider profile is `openai-compatible` or `custom`
- **AND** it is not recognized as official OpenAI or ForOpenCode
- **AND** the profile image API compatibility is `auto`
- **WHEN** the system infers image compatibility
