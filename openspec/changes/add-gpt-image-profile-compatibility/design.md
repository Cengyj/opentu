...
> `for-gpt-image` value is accepted only as an alias for `for-gpt-image`,
> For GPT Image routes through the dedicated For GPT Image adapter, and
...
- `model-adapters/default-adapters.ts`
  - Continues to serve For/basic and generic OpenAI-compatible image formats.
...
The existing default adapter remains the compatibility path for generic OpenAI-compatible image providers. For GPT Image-specific behavior belongs to the dedicated For GPT Image adapter. Internally, the compatibility identifier remains `for-gpt-image` for historical configuration compatibility.
...
- Legacy default and managed ForOpenCode profiles should store `auto`.
- At runtime, `auto` for `foropencode.com` resolves to `for-gpt-image`, preserving current behavior.
...
- Default adapter changes may break existing For/basic image keys.
...
2. Add routing/schema split while defaulting For/basic profiles to the existing adapter.
...
- Should `foropencode.com` always resolve to `for-gpt-image` in `auto` mode?
- Should the first implementation include only `/images/generations`, or also `/images/edits`?
- Should `openai-compatible-basic` and `for-gpt-image` remain separate user options even if they initially map to the same request schema?
