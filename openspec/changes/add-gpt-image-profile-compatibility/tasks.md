...
- [x] 2.4 Ensure legacy default and managed ForOpenCode profiles store `auto`.
- [x] 2.5 Ensure `auto` for ForOpenCode profiles resolves to the existing compatibility path at runtime.
...
- [x] 3.4 Keep `for-gpt-image` and `openai-compatible-basic` on `openai.image.basic-json`.
...
- [x] 5.3 Use user-facing labels for automatic, OpenAI GPT Image, For-compatible, and generic OpenAI-compatible formats.
...
- [x] 7.3 Add default adapter regression tests for existing For/basic request bodies.
...
- [ ] 8.1 Create one `for-gpt-image` profile and one `openai-gpt-image` profile that both select `gpt-image-2`.
...
- [ ] 9.2 Confirm ForOpenCode/New API `auto` resolves to the dedicated For GPT Image adapter path.
