...
The same `gpt-image-*` model ID can require different image API request formats depending on the selected provider profile and API key. Some keys need the existing For/basic image compatibility path, while others need the official OpenAI GPT Image format.
...
- Keep the current default image adapter compatibility path for For/basic and generic OpenAI-compatible image requests.
